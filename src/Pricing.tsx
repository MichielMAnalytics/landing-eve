  ...
  return (
    <div className="min-h-screen lightblue-grid-bg">
      ...
      <section className="py-32">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-8 md:px-16 lg:px-24 xl:px-32 grid md:grid-cols-3 gap-8 mt-12">
          {pricingPlans.map((plan, idx) => (
            <div
              ...
              aria-label={plan.popular ? 'Most popular plan' : undefined}
            >
              {plan.popular && (
                <div className="mb-4 px-4 py-1 rounded-full bg-[#FFFFFF]/10 text-[#FFFFFF] font-inter text-sm font-semibold border border-[rgba(216,217,236,0.2)]">
                  Most Popular
                </div>
              )}
              <h2 className="text-2xl font-comfortaa font-bold text-[#FFFFFF] mb-2">{plan.name}</h2>
              ...
              <ul className="mb-8 space-y-4 text-left w-full max-w-xs mx-auto">
                {plan.features.map((feature, i) => (
                  <li key={i} className="flex items-center text-[#FFFFFF]/90 font-inter">
                    <Check className="w-5 h-5 mr-3 text-[#FFFFFF]" />
                    {feature}
                  </li>
                ))}
              </ul>
      ...
      <section className="py-20">
        <div className="max-w-4xl mx-auto px-4">
          ...
          <div className="space-y-6">
            {faqs.map((faq, idx) => (
              <div key={idx} className="bg-[#04062D] border-2 border-[rgba(216,217,236,0.2)] rounded-xl p-6 hover:border-[rgba(216,217,236,0.3)] transition-all duration-300">
